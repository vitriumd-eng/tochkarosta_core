...
async def get_tenant_database_url(tenant_id: str, module_id: str) -> Dict:
    ...
    # Remove +asyncpg for parsing
    sync_url = database_url.replace("postgresql+asyncpg://", "postgresql://", 1)
    pattern = r'postgresql://([^:]+):([^@]+)@([^:/]+)(?::(\d+))?/([^/?]+)'
    match = re.match(pattern, sync_url)
    
    if not match:
        raise ValueError(f"Invalid core DATABASE_URL format: {sync_url.split('@')[0]}@***")
...
async def notify_tenant(tenant_id: str, payload: Dict) -> None:
    ...
    Args:
        ...
        payload: Notification payload with keys:
            - type: 'email' | 'sms' | 'push'
            - subject: Subject/title (optional)
            - message: Message content
            - recipient: Email/phone/user_id (optional, uses tenant owner if not provided)
    """
    try:
        async with AsyncSessionLocal() as db:
            tenant_uuid = uuid.UUID(tenant_id)
            ...
            if not tenant:
                logger.warning(f"Tenant {tenant_id} not found for notification")
                return
            
            notification_type = payload.get("type", "email")
            message = payload.get("message", "")
            subject = payload.get("subject", "")
            recipient = payload.get("recipient", tenant.owner_phone)
            
            # Log notification (in production, integrate with actual services)
            logger.info(
                f"Notification to tenant {tenant_id} ({tenant.name}): "
                f"type={notification_type}, recipient={recipient}, subject={subject}"
            )
            
            # TODO: Integrate with actual notification services
            # For email: SendGrid, AWS SES, etc.
            # For SMS: Twilio, AWS SNS, etc.
            # For push: Firebase, AWS SNS, etc.
            
            if notification_type == "email":
                # Placeholder for email service integration
                logger.debug(f"Email notification: to={recipient}, subject={subject}, message={message[:50]}...")
            elif notification_type == "sms":
                # Placeholder for SMS service integration
                logger.debug(f"SMS notification: to={recipient}, message={message[:50]}...")
            elif notification_type == "push":
                # Placeholder for push notification service integration
                logger.debug(f"Push notification: to={recipient}, message={message[:50]}...")
            else:
                logger.warning(f"Unknown notification type: {notification_type}")
                
    except Exception as e:
        logger.error(f"Failed to send notification to tenant {tenant_id}: {e}", exc_info=True)
        # Don't raise exception - notifications should not break main flow
